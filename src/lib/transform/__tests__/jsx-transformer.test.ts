...
test("transformJSX transforms TypeScript files with correct presets", async () => {
  const code = `const Component = () => <div>Hello</div>;`;
  const result = await transformJSX(code, "test.tsx", new Set());
...
test("transformJSX handles JavaScript files without TypeScript preset", async () => {
  const code = `const Component = () => <div>Hello</div>;`;
  const result = await transformJSX(code, "test.jsx", new Set());
...
test("transformJSX collects imports from code", async () => {
  const code = `
...
test("transformJSX handles transform errors gracefully", async () => {
  // Mock Babel to throw an error
  vi.mocked(Babel.transform).mockImplementationOnce(() => {
    throw new Error("Transform failed");
  });

  const result = await transformJSX("invalid code", "test.jsx", new Set());
...
test("createImportMap includes React CDN imports", async () => {
  const files = new Map();
  const result = await createImportMap(files);
...
test("createImportMap transforms JavaScript and TypeScript files", async () => {
  const files = new Map([
...
test("createImportMap creates multiple path variations for files", async () => {
  const files = new Map([
...
test("createImportMap creates placeholder modules for missing imports", async () => {
  const files = new Map([
...
test("createImportMap handles @/ alias imports", async () => {
  const files = new Map([
...
test("integration: full transformation pipeline works", async () => {
  const files = new Map([
    ["/App.jsx", `
      import React from 'react';
...
// CSS Support Tests
test("transformJSX detects CSS imports", async () => {
  const code = `
    import React from 'react';
    import './styles.css';
  ...
  `;
  const result = await transformJSX(code, "App.jsx", new Set());
...
test("transformJSX removes CSS imports from transformed code", async () => {
  const code = `
    import React from 'react';
    import './styles.css';
...
test("transformJSX handles CSS imports with different quotes", async () => {
  const code = `
    import './single.css';
    import "./double.css";
    import '@/styles/globals.css';
...
test("createImportMap collects CSS files and returns styles", async () => {
  const files = new Map([
...
test("createImportMap handles missing CSS files gracefully", async () => {
  const files = new Map([
...
test("createImportMap resolves CSS import paths correctly", async () => {
  const files = new Map([
...
test("integration: full pipeline handles components with CSS imports", async () => {
  const files = new Map([
    ["/App.jsx", `
      import React from 'react';
      import './App.css';
...
test("createImportMap handles syntax errors gracefully", async () => {
...
test("files with syntax errors are not included in import map", async () => {
  // Mock Babel to throw error for BadComponent
  vi.mocked(Babel.transform).mockImplementation((code, options) => {
    if (options.filename === "/BadComponent.jsx") {
